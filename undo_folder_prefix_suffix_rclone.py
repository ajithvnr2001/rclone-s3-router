"""
Undo Folder Prefix/Suffix (fol...name...fol) - RCLONE VERSION

Recursively REMOVES "fol..." prefix and "...fol" suffix from folder names.
...
S3_REMOTE = "wasabi"
S3_LOG_PATH = "data-migration-logs/folder_rename_undo_progress.log"

# Local temp file
LOCAL_LOG = "/tmp/folder_rename_undo_progress.log"

# Prefix and suffix to remove
PREFIX = "fol..."
SUFFIX = "...fol"
...
def log_done(old_path, new_path):
    ...
    pending_logs.append(f"UNDONE|{old_path}|{new_path}")
    if len(pending_logs) >= LOG_SYNC_INTERVAL:
        upload_log()
        print(f"  📤 Log synced to S3 ({stats['renamed']} undone so far)")
        ...
        with open(LOCAL_LOG, "r") as f:
            for line in f:
                ...
                if line.startswith("UNDONE|"):
                    parts = line.split("|", 2)
                    if len(parts) >= 2:
                        done.add(parts[1])  # old path (with prefix/suffix)
        print(f"✅ Resume: {len(done)} already undone from previous run")
...
def has_prefix_suffix(name):
    """Check if folder name has the fol...X...fol pattern."""
    return name.startswith(PREFIX) and name.endswith(SUFFIX)


def strip_prefix_suffix(name):
    """Remove fol... prefix and ...fol suffix to get original name."""
    if has_prefix_suffix(name):
...
def main():
    print("=" * 60)
    print("🔄 UNDO FOLDER PREFIX/SUFFIX")
    print(f"📁 Remote: {REMOTE}:{ROOT_PATH}")
    print(f"🏷️  Removing: {PREFIX}<name>{SUFFIX} → <name>")
    print("=" * 60)
    ...
    for folder in folders:
        ...
        # Skip if folder does NOT have the prefix/suffix (already undone or never renamed)
        if not has_prefix_suffix(name):
            stats["skipped"] += 1
            continue
        ...
        # Strip prefix/suffix to get original name
        original_name = strip_prefix_suffix(name)
        parent = os.path.dirname(full_path)
        new_path = f"{parent}/{original_name}" if parent else original_name
        ...
        if ok:
            ...
            if stats["renamed"] <= 5 or stats["renamed"] % 100 == 0:
                print(f"  ✅ [{stats['renamed']}] {name} → {original_name}")
        else:
            stats["errors"] += 1
            print(f"  ❌ FAIL: {name} → {original_name} | {err}")
