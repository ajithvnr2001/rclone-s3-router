#!/usr/bin/env python3
"""
PYTHON MASTER WORKER (v3 - Fully Fixed + All v2 Bugs Resolved)
Features:
...
- Fixed skipped count calculation error in resume logic
...
- Added validation for S3 upload success
- Fixed ThreadPoolExecutor max_workers calculation
- Added orphaned temp directory cleanup on startup
- Large file transfer now checks rclone availability first
- Consistent progress file naming
...
def transfer_large_files(folder_name: str, status_queue, lock) -> List[str]:
    ...
    # V3 FIX: Check rclone availability first
    ...
    for i, lf in enumerate(remaining):
        # V3 FIX: Check for shutdown request
...
def pipeline_worker(task_data) -> bool:
    ...
    if completed_files:
        # V3 FIX: Correct skipped count calculation
        ...
        try:
            ...
            if downloaded_files:
                ...
                if os.path.exists(local_zip):
                    ...
                    # V3 FIX: Upload with retry and validation
                    def _upload():
...
def main():
    print("🚀 PYTHON MASTER WORKER (v3 - Fully Fixed)")
    print("=" * 60)
    ...
    # Install zip if needed (V3 FIX: No shell=True)
    try:
        subprocess.run(
            ["apt-get", "update"],
    ...
    if shutil.which("rclone") is None:
        ...
        # V3 FIX: Use subprocess list format instead of shell=True where possible
        # Note: rclone install requires piped shell command, but we validate the source
        try:
            # Download and verify install script first (more secure)
    ...
    # V3 FIX: Clean up orphaned temp directories from previous crashes
    ...
    for folder in SUBFOLDERS:
        ...
        # Run tasks
        has_failures = False
        # V3 FIX: Correct ThreadPoolExecutor max_workers (not +1)
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_PARALLEL_WORKERS) as thread_exe:
            ...
            if files:
                ...
                # V3 FIX: Capture variables properly in closure
                files_copy = files[:]
                folder_copy = folder
                
                def run_zip_pipeline(files_list=files_copy, folder_name=folder_copy):
                    tasks = []
                    for i in range(num_parts):
                        ...
                        tasks.append((batch, f"{SOURCE}/{folder_name}", s3_key, part, folder_name, q, lock))
